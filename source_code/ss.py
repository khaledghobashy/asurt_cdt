...
def acc_rhs(q,qdot,bodies,joints,actuators): 
	 ...
	 system=np.bmat([[joints['rev'].acc_rhs(q,qdot)],[bodies['ground'].mount_acc_rhs(qdot)],[bodies['link'].acc_rhs(qdot)],[actuators['rot'].acc_rhs(q,qdot)]]) 
	 return sparse.csc_matrix(np.array(system,dtype=np.float64).reshape((14,1)))
