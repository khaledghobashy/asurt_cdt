...
jac_df=pd.DataFrame([2 *[None]],columns=['rbs_ground', 'rbs_link'],index=['jcs_rev', 'rbs_ground', 'rbs_link', 'mcs_act'])
def cq(q,bodies,joints,actuators): 
	 ...
	 jac_df.loc['mcs_act','rbs_ground']=actuators['mcs_act'].jacobian_i(q)
	 jac_df.loc['mcs_act','rbs_link']=actuators['mcs_act'].jacobian_j(q)
	 jac=sparse.bmat(jac_df,format='csc') 
	 return jac 


eq_s=pd.Series([4 *[None]],index=['jcs_rev', 'rbs_ground', 'rbs_link', 'mcs_act'])
def eq(q,bodies,joints,actuators): 
	 ...
	 eq_s['mcs_act']=actuators['mcs_act'].equations(q)
	 system=sparse.bmat(eq_s.values.reshape((4,1)),format='csc') 
	 return system.A.reshape((14,)) 


vel_rhs_s=pd.Series([4 *[None]],index=['jcs_rev', 'rbs_ground', 'rbs_link', 'mcs_act'])
def vel_rhs(actuators): 
	 vrhs=np.zeros((13,1))
	 vrhs=np.concatenate([vrhs,actuators['mcs_act'].vel_rhs()]) 
	 return vrhs 
acc_rhs_s=pd.Series([4 *[None]],index=['jcs_rev', 'rbs_ground', 'rbs_link', 'mcs_act'])
def acc_rhs(q,qdot,bodies,joints,actuators): 
	 ...
	 acc_rhs_s['mcs_act']=actuators['mcs_act'].acc_rhs(q,qdot)
