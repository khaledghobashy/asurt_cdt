...
jac_df=pd.DataFrame([2 *[None]],columns=['rbs_ground', 'rbs_link'],index=['jcs_rev', 'rbs_ground', 'rbs_link', 'act'])
...
eq_s=pd.Series([4 *[None]],index=['jcs_rev', 'rbs_ground', 'rbs_link', 'act'])
...
vel_rhs_s=pd.Series([4 *[None]],index=['jcs_rev', 'rbs_ground', 'rbs_link', 'act'])
def vel_rhs(actuators): 
	 vrhs=np.zeros((13,1))
	 vrhs=np.concatenate([vrhs,actuators['act'].vel_rhs()]) 
	 return vrhs 
acc_rhs_s=pd.Series([4 *[None]],index=['jcs_rev', 'rbs_ground', 'rbs_link', 'act'])
