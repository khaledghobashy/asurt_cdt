...
jac_df=pd.DataFrame([4 *[None]],columns=['rbs_connecting', 'rbs_cranck', 'rbs_ground', 'rbs_slider'],index=['jcs_A_sph', 'jcs_B_uni', 'jcs_C_trans', 'jcs_O_rev', 'rbs_connecting', 'rbs_cranck', 'rbs_ground', 'rbs_slider', 'act'])
def cq(q,bodies,joints,actuators): 
	 ...
	 jac_df.loc['act','rbs_cranck']=actuators['act'].jacobian_i(q)
	 jac_df.loc['act','rbs_ground']=actuators['act'].jacobian_j(q)
	 jac=sparse.bmat(jac_df,format='csc') 
	 return jac 


eq_s=pd.Series([9 *[None]],index=['jcs_A_sph', 'jcs_B_uni', 'jcs_C_trans', 'jcs_O_rev', 'rbs_connecting', 'rbs_cranck', 'rbs_ground', 'rbs_slider', 'act'])
...
vel_rhs_s=pd.Series([9 *[None]],index=['jcs_A_sph', 'jcs_B_uni', 'jcs_C_trans', 'jcs_O_rev', 'rbs_connecting', 'rbs_cranck', 'rbs_ground', 'rbs_slider', 'act'])
def vel_rhs(actuators): 
	 vrhs=np.zeros((27,1))
	 vrhs=np.concatenate([vrhs,actuators['act'].vel_rhs()]) 
	 return vrhs 
acc_rhs_s=pd.Series([9 *[None]],index=['jcs_A_sph', 'jcs_B_uni', 'jcs_C_trans', 'jcs_O_rev', 'rbs_connecting', 'rbs_cranck', 'rbs_ground', 'rbs_slider', 'act'])
